...
| **7** | Wrap-up | **In progress.** Single-command entrypoint built and verified live: `scripts/reconcile.py` now builds and writes the graph as its own first step (closing a real gap against the brief's "must start with a single documented command" requirement — previously needed three separate commands in a specific order). `README.md` rewritten with real, verified setup and evaluation steps, plus an honest "known scope limits" section. **Verified: `python3 scripts/reconcile.py --firm firm_a` then `--firm firm_b`, back to back, both `OVERALL: PASS`** — including confirming the idempotent graph rebuild on the second run didn't duplicate anything (same 85 nodes / 100 edges / 185 statements both times). **Not yet done, precisely scoped rather than assumed:** this proves the single-command flow works in the existing dev environment, not from a genuinely fresh clone (new directory, no `.venv`, no cached `__pycache__`) — that stricter test is still open. Also still open: formal byte-identical-JSON determinism test, missing-provenance live test, and the bonus items (gated on being here, which it now is). |
